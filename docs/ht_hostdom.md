...
NOTE: This requires that you have a static Internet IP address. You cannot reliably host DNS on a dynamic IP address (such as a dial-up connection).
...
NOTE: The registrar may do a DNS lookup to see if you have the DNS server name listed correctly on your DNS server. So before you use a new DNS servers name ("ns#.example.com") for the first time in a domain name registration, make sure to first set up an [NS-record](rec_ns.md) and associated [A-record](rec_a.md) for this in Simple DNS Plus.
